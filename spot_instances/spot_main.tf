  ...
  provisioner "file" {
   ...
   provisioner "remote-exec" {
    inline = [
      ...
      "kubectl apply -f 'https://install.portworx.com/2.0.2?mc=false&kbver=1.13.3&b=true&c=px-demo&stork=true&lh=true&mon=true&st=k8s'",
...
output "lighthouse_url" {
    value = ["http://${aws_spot_instance_request.worker.0.public_ip}:32678"]
}

output "grafana_url" {
    value = ["http://${aws_spot_instance_request.worker.0.public_ip}:30950"]
}
